...
local pacupdates = awful.widget.watch('bash -c "checkupdates | wc -l"', 90, function(widget, stdout) -- every 90 seconds it checks for updates
    widget:set_markup(markup.fontfg(theme.font, "#4B3B51", "" .. stdout .. " "))
end)
local aurupdates = awful.widget.watch('bash -c "paru -Qua | wc -l"', 90, function(widget, stdout) -- every 90 seconds it checks for updates
    widget:set_markup(markup.fontfg(theme.font, "#4B3B51", "+" .. stdout .. " "))
end)
    ...
    s.mywibox:setup {
        ...
        { -- Right widgets
           ...
           -- wibox.container.background(wibox.container.margin(wibox.widget { mailicon, mail and mail.widget, layout = wibox.layout.align.horizontal }, 4, 7), "#343434"),
            ...
            arrow("#4B3B51", "#CB755B"),
            wibox.container.background(wibox.container.margin(wibox.widget { pacicon, pacupdates,aurupdates ,layout = wibox.layout.align.horizontal }, 2, 3), "#CB755B"),
            ...
            arrow("#8DAA9A", "#B0BC83"),
            wibox.container.background(wibox.container.margin(wibox.widget { nil, net.widget, layout = wibox.layout.align.horizontal }, 3, 3), "#B0BC83"),
            arrow("#B0BC83", "#777E76"),
